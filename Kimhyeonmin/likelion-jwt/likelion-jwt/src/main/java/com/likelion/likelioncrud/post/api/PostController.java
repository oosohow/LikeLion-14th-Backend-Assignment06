...
public class PostController {
    ...
    // 게시물 저장
    @PostMapping()
    @Operation(summary = "게시물 저장", description = "BE 파트만 게시글을 작성할 수 있음")
    ...
    public ApiResTemplate<Page<PostInfoResponseDto>> myPostFindAll(
            //AI 파트 조회 막기 위해 조회 요청 userId 받기
            @AuthenticationPrincipal Long userId,
            @PathVariable("memberId") Long memberId,
            @ParameterObject
            @PageableDefault(size = 10, sort = "postId", direction = Sort.Direction.ASC) Pageable pageable) {
        Page<PostInfoResponseDto> posts = postService.postFindMember(userId, memberId, pageable); //userId, memberId 모두 서비스로 넘김
        return ApiResTemplate.successResponse(SuccessCode.GET_SUCCESS, posts);
    }

    // 게시물 id를 기준으로 사용자가 작성한 게시물 수정
    @PatchMapping("/{postId}")
    @Operation(summary = "게시물 Id로 수정", description = "BE 파트만 게시글을 수정할 수 있음")
    ...
    @Operation(summary = "게시물 삭제", description = "BE 파트만 게시글을 삭제할 수 있음")
